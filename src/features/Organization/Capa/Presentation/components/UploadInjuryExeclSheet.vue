...
import AddInjuryExcelParams from '../../Core/params/AddInjuryExcelParams';
...
// ─── State ────────────────────────────────────────────────────────────────────
const sheetData = ref<InjuryModel[] | null>(null);
...
const getBodyData = (data: any[]) =>
  InjuryModel.transformData(data.slice(1));
  ...
  <div class="page-wrapper">
    ...
    <template v-else>
      ...
      <template v-if="mappedData && mappedData.length > 0">
        ...
        <button @click="AddInjuries" class="btn-confirm">
          Confirm & Submit
        </button>
      </template>
    </template>

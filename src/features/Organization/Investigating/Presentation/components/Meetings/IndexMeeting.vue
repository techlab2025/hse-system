...
const indexMeetingController = IndexMeetingController.getInstance()
const state = ref(indexMeetingController.state.value)
const route = useRoute()
...
const fetchMeetings = async (
  ...
  const deleteEquipmentTypeParams = new IndexMeetingParams(
    query,
    pageNumber,
    perPage,
    withPage,
...
const handleChangePage = (page: number) => {
  ...
  fetchMeetings('', currentPage.value, countPerPage.value)
}
...
watch(
  () => indexMeetingController.state.value,
  (newState) => {
    if (newState) {

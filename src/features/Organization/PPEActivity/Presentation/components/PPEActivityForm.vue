...
const emit = defineEmits<{
  (event: 'update:data', value: AddPPEActivityParams | EditPPEActivityParams): void
}>()
const props = defineProps<{ data?: PPEActivityDetailsModel }>()
...
const updateData = () => {
  ...
  emit(
    'update:data',
    props.data?.id
      ? new EditPPEActivityParams(props.data.id, translations, adminScope, industryIds)
      : new AddPPEActivityParams(translations, adminScope, industryIds),
  )
}
...
const requiredFields = computed(() => [
  {
    key: 'title',
    message: t('ppe_activity_title_required'),
...
<template>
  <div class="col-span-4 md:col-span-2" data-required-field="title">
    <LangTitleInput
      :langs="languages"
      :model-value="titles"
      :label="$t('ppe_activity_title')"
      :placeholder="$t('enter_ppe_activity_title')"
  ...
  <div v-if="!allIndustries && user.user?.type === OrganizationTypeEnum.ADMIN" class="col-span-4 md:col-span-2">
    <CustomSelectInput :model-value="industry" :controller="industryController" :params="industryParams"
      label="industry" id="ppe-activity-industry" placeholder="Select industry" :type="2"

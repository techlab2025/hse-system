...
// ─── Helpers ──────────────────────────────────────────────────────────────────
const getBodyData = (data: any[]) => PPEActivityModel.transformData(data.slice(1))
...
const addPPEActivities = async () => {
  ...
  const orgData = new AddPPEActivityExcelParams({ data: dataAsObjects })
  await addPPEActivityController.addPPEActivity(orgData, router)

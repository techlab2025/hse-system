...
import EditPPEToolParams from '../../Core/params/editPPEToolParams'
...
const emit = defineEmits<{
  (event: 'update:data', value: AddPPEToolParams | EditPPEToolParams): void
}>()
...
const updateData = () => {
  ...
  emit(
    'update:data',
    props.data?.id
      ? new EditPPEToolParams(props.data.id, translations, adminScope, industryIds)
      : new AddPPEToolParams(translations, adminScope, industryIds),
  )
}
...
<template>
  <div class="col-span-4 md:col-span-2" data-required-field="title">
    <LangTitleInput
      :langs="languages"
      :model-value="titles"
      :label="$t('ppe_tool_title')"
      :placeholder="$t('enter_ppe_tool_title')"
  ...
  <div v-if="!allIndustries && user.user?.type === OrganizationTypeEnum.ADMIN" class="col-span-4 md:col-span-2">
    <CustomSelectInput :model-value="industry" :controller="industryController" :params="industryParams"
      label="industry" id="ppe-tool-industry" placeholder="Select industry" :type="2"

...
// ─── State ────────────────────────────────────────────────────────────────────
const sheetData = ref<PPEToolModel[] | null>(null)
const Data = ref<any[]>([])
...
// ─── Helpers ──────────────────────────────────────────────────────────────────
const getBodyData = (data: any[]) => PPEToolModel.transformData(data.slice(1))
...
const SendDataLabels = computed(() => ({ title: t('ppe_tool_title') }))
...
const addPPETools = async () => {
  ...
  const orgData = new AddPPEToolExcelParams({ data: dataAsObjects })
  await addPPEToolController.addPPETool(orgData, router)
    ...
    <div class="page-wrapper">
      ...
      <template v-else>
        ...
        <template v-if="mappedData && mappedData.length > 0">
          ...
          <button class="btn-confirm" @click="addPPETools">{{ $t('confirm_and_submit') }}</button>
        </template>
      </template>
    </div>
  </PermissionBuilder>

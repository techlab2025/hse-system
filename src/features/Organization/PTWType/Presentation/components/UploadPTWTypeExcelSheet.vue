...
// ─── State ────────────────────────────────────────────────────────────────────
const sheetData = ref<PTWTypeModel[] | null>(null)
const Data = ref<any[]>([])
...
// ─── Helpers ──────────────────────────────────────────────────────────────────
const getBodyData = (data: any[]) => PTWTypeModel.transformData(data.slice(1))
...
const SendDataLabels: Record<string, string> = {
  title: 'Permit To Work Type Title',
}
...
const addPTWTypes = async () => {
  ...
  const orgData = new AddPTWTypeExcelParams({ data: dataAsObjects })
  await addPTWTypeController.addPTWType(orgData, router)
  ...
  <div class="page-wrapper">
    ...
    <template v-else>
      ...
      <template v-if="mappedData && mappedData.length > 0">
        ...
        <button class="btn-confirm" @click="addPTWTypes">Confirm & Submit</button>
      </template>
    </template>
  </div>
</template>

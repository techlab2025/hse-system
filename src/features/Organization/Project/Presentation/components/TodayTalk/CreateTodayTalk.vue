...
import { DataFailed, DataLoading } from '@/base/core/networkStructure/Resources/dataState/data_state'
...
const isEmployeesLoading = computed(() => employeesState.value instanceof DataLoading)
const hasEmployeeError = computed(() => employeesState.value instanceof DataFailed)
...
const validate = () => {
  ...
  employeesError.value = attendance.value.length ? '' : 'No project employees are available'
  return !contentError.value && !employeesError.value
}
...
watch(
  ...
  (locations) => hydrateEmployees(locations ?? null),
  { immediate: true },
)
  ...
  <main class="today-talk-page">
    ...
    <form class="talk-form" @submit.prevent="saveTalk">
      ...
      <section class="attendance-card">
        ...
        <div v-else class="employees-empty">
          ...
          <div>
            ...
            <p>Add employees to this project or retry loading the project workforce.</p>
          </div>
      ...
      <footer class="form-actions">
        <RouterLink :to="`/organization/project-details/${projectId}`" class="cancel-button">
          Cancel
        </RouterLink>
...
.heading-row p,
.section-heading p,
.employees-empty p { margin: 0; color: var(--text-soft); line-height: 1.55; }
...
.attendance-actions { display: flex; align-items: center; gap: 12px; color: var(--text-soft); font-size: 0.8rem; }
.attendance-actions strong { color: var(--PrimaryColor); font-size: 1.05rem; }
...
.select-all { padding: 9px 12px; }
.employee-grid { display: grid; grid-template-columns: repeat(2, minmax(0, 1fr)); gap: 12px; margin-top: 21px; }
...
.employee-card:hover { transform: translateY(-2px); border-color: color-mix(in srgb, var(--PrimaryColor) 45%, var(--main-border)); }
.employee-card.attending { border-color: color-mix(in srgb, var(--PrimaryColor) 50%, var(--main-border)); background: color-mix(in srgb, var(--PrimaryColor) 7%, var(--surface-1)); }
.avatar { display: grid; width: 44px; height: 44px; place-items: center; overflow: hidden; border-radius: 14px; color: var(--PrimaryColor); background: color-mix(in srgb, var(--PrimaryColor) 12%, var(--surface-1)); font-size: 0.78rem; font-weight: 900; }
.avatar img { width: 100%; height: 100%; object-fit: cover; }
.employee-copy { display: flex; min-width: 0; flex-direction: column; }
.employee-copy strong,
.employee-copy small { overflow: hidden; text-overflow: ellipsis; white-space: nowrap; }
.employee-copy strong { color: var(--text-strong); font-size: 0.88rem; }
.employee-copy small,
.attendance-control small { color: var(--text-soft); font-size: 0.7rem; }
.attendance-control { display: flex; align-items: center; flex-direction: column; gap: 4px; }
.employee-skeleton { height: 72px; border-radius: 15px; background: color-mix(in srgb, var(--PrimaryColor) 9%, var(--surface-2)); animation: pulse 1.2s ease-in-out infinite alternate; }
.employees-empty { gap: 13px; margin-top: 20px; padding: 18px; border: 1px dashed var(--main-border); border-radius: 15px; background: var(--surface-2); }
.employees-empty > span { display: grid; width: 38px; height: 38px; flex: 0 0 38px; place-items: center; border-radius: 50%; color: var(--PrimaryColor); background: color-mix(in srgb, var(--PrimaryColor) 12%, transparent); font-weight: 900; }
.employees-empty div { flex: 1; }
.employees-empty h3 { margin: 0 0 3px; color: var(--text-strong); font-size: 0.95rem; }
.employees-empty p { font-size: 0.78rem; }
.retry { padding: 8px 13px; }
...
@media (max-width: 760px) {
  ...
  .attendance-actions { width: 100%; justify-content: space-between; }
}
...
@media (max-width: 520px) {
  ...
  .employee-card { grid-template-columns: auto minmax(0, 1fr); }
  .attendance-control { grid-column: 1 / -1; align-items: center; flex-direction: row; justify-content: flex-end; }
}
</style>

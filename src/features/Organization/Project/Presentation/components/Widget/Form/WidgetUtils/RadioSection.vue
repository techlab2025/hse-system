  ...
  <div class="radio-selection-containre">
    <div class="radio-selection-header">
      ...
      <div class="checkbox-header" v-if="Customize">
        <input type="checkbox" v-model="customize" :id="title.split(' ').join('-')" :name="title.split(' ').join('-')" />
        <label class="checkbox-label" :for="title.split(' ').join('-')">customize</label>
      </div>
    </div>

    <div class="selection-container" v-if="selections && selections.length > 0">
      <div class="selection" :class="SelectedOption == select ? 'selected' : ''" v-for="(select, index) in selections"
        :key="index">

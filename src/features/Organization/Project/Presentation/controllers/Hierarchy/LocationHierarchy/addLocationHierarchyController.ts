...
export default class AddLocationHierarchyController extends ControllerInterface<LocationHierarchyModel> {
  ...
  private AddLocationHierarchyUseCase = new AddLocationHierarchyUseCase()

  static getInstance() {
    if (!this.instance) {
      this.instance = new AddLocationHierarchyController()
    }
    return this.instance
  }

  async addLocationHierarchy(params: Params, router: Router, draft: boolean = false) {
    // useLoaderStore().setLoadingWithDialog();
    try {
      const dataState: DataState<LocationHierarchyModel> =
        await this.AddLocationHierarchyUseCase.call(params)

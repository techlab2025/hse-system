...
export default class CreateProjectMeetingResultController extends ControllerInterface<ProjectModel> {
  ...
  private createProjectMeetingResultUseCase = new CreateProjectMeetingResultUseCase()

  static getInstance() {
    if (!this.instance) {
      this.instance = new CreateProjectMeetingResultController()
    }
    return this.instance
  }

  async CreateProjectMeetingResult(params: Params, router: Router, draft: boolean = false) {
    // useLoaderStore().setLoadingWithDialog();
    try {
      const dataState: DataState<ProjectModel> =
        await this.createProjectMeetingResultUseCase.call(params)

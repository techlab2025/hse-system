...
import EditVisitCategoryParams from '../../Core/params/editVisitCategoryParams'
...
const emit = defineEmits<{
  (event: 'update:data', value: AddVisitCategoryParams | EditVisitCategoryParams): void
}>()
const props = defineProps<{ data?: VisitCategoryDetailsModel }>()
...
const updateData = () => {
  ...
  emit(
    'update:data',
    props.data?.id
      ? new EditVisitCategoryParams(props.data.id, translations, adminScope, industryIds)
      : new AddVisitCategoryParams(translations, adminScope, industryIds),
  )
}
...
const requiredFields = computed(() => [
  {
    key: 'title',
    message: 'Visit Category Title Is Required',
    isMissing: () =>
      !titles.value.some((item) => String(item.title ?? '').trim().length > 0),
  },
])
...
<template>
  <div class="col-span-4 md:col-span-2" data-required-field="title">
    <LangTitleInput
      :langs="languages"
      :model-value="titles"
      :label="$t('visit_category_title')"
      :placeholder="$t('enter_visit_category_title')"
  ...
  <div v-if="!allIndustries && user.user?.type === OrganizationTypeEnum.ADMIN" class="col-span-4 md:col-span-2">
    <CustomSelectInput :model-value="industry" :controller="industryController" :params="industryParams"
      label="industry" id="visitcategory-industry" placeholder="Select industry" :type="2"

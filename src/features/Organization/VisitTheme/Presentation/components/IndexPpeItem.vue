...
import IndexPpeItemParams from '../../Core/params/indexVisitThemeParams.ts'
import DeletePpeItemParams from '../../Core/params/deleteVisitThemeParams.ts'
...
<template>
  <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 mb-4">
    <div class="input-search col-span-1">
      <span class="icon-remove" @click="((word = ''), searchPpeItems())">
        <Search />
      </span>
      <input v-model="word" :placeholder="$t('search')" class="input" @input="searchPpeItems" />
    </div>
    <div class="col-span-2 flex justify-end gap-2">
      <ActionsList feature-name="action_feature_ppe_items" :show-actions="true" :action-list="headerActions()"
        :actions-number="5">
        <template #custom>
          <ExportPdf :is-drop-list="true" />
        </template>
      </ActionsList>
    </div>
    <SystemPpeItems v-if="user?.type !== OrganizationTypeEnum.ADMIN" :is-header-tap="true" />
  </div>
  ...
  <PermissionBuilder :code="featurePermissions">
    <DataStatus :controller="state">
          ...
          <table class="main-table">
            ...
            <tbody>
              <tr v-for="(item, index) in state.data" :key="item.id">
                ...
                <td>{{ item.title }}</td>
                <td>
                  <DropList :action-list="rowActions(item.id)" />
                </td>
              </tr>
            </tbody>
          </table>
        </div>
        <Pagination :pagination="state.pagination" @change-page="changePage" @count-per-page="changePageSize" />
      </template>
      <template #loader>
      ...
      <template #empty>
        <DataEmpty :link="`${basePath}/ppe-item/add`" :add-text="$t('add_ppe_item')"
          :description="$t('no_ppe_items_description')" :title="$t('no_ppe_items')" />
      </template>
      <template #failed>
        <DataFailed :link="`${basePath}/ppe-item/add`" :add-text="$t('add_ppe_item')"
          :description="$t('no_ppe_items_description')" :title="$t('no_ppe_items')" />
      </template>
    </DataStatus>
  ...
  <Dialog v-model:visible="showUploadDialog" modal :dismissable-mask="true" :header="$t('import_ppe_items')"
    :style="{ width: '80vw', maxWidth: '900px' }">
    <UploadPpeItemExcelSheet :initial-file="pendingFile" @uploaded="onUploaded" />
  </Dialog>

...
const indexPpeItemController = IndexPpeItemController.getInstance()
const state = ref(indexPpeItemController.state.value)

const fetchPPEItem = async () => {
  const params = new IndexPpeItemParams('', 1, 10, 1)
  await indexPpeItemController.getData(params)
}
onMounted(fetchPPEItem)
...
const exportExcel = () => {
  ...
  saveAs(data, 'root_ppe_item_form.xlsx')
}

const DownloadExample = () => {
  const worksheetData = [{ title: 'Example title' }, { title: 'Example title 2' }]
  const worksheet = XLSX.utils.json_to_sheet(worksheetData)
  const workbook = XLSX.utils.book_new()
  XLSX.utils.book_append_sheet(workbook, worksheet, 'RootCauses')
  const excelBuffer = XLSX.write(workbook, { bookType: 'xlsx', type: 'array' })
  const blob = new Blob([excelBuffer], { type: 'application/octet-stream' })
  saveAs(blob, 'ppe_item_form.xlsx')
}
...
<template>
  <div class="flex justify-end gap-2">
    <ActionsList feature-name="action_feature_ppe_item" :show-actions="true" :actionList="actionList()"
      :actionsNumber="2">
    </ActionsList>
  </div>

  <Dialog v-model:visible="showUploadDialog" modal :dismissable-mask="true" :header="$t('import_ppe_item')"
    :style="{ width: '80vw', maxWidth: '900px' }">
    <UploadPpeItemExcelSheet :initial-file="pendingFile" @uploaded="

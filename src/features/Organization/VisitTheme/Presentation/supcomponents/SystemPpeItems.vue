      ...
      <template #success>
        ...
        <div v-else class="system-dialog-content-container">
          ...
          <div class="system-dialog-content" v-for="item in state.data" :key="item.id">
            <div class="row-content" :class="{ active: selectedIds.includes(item.id) }"
        ...
        <button v-if="state.data?.length" class="btn btn-primary w-full mt-5 confirm-btn" @click="clonePpeItems">{{
          $t('confirm') }}</button>
      </template>
      <template #empty>

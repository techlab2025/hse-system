...
const state = ref(indexWhereHouseController.state.value)
const route = useRoute()


const fetchWhereHouse = async (
  query: string = '',
  pageNumber: number = 1,
  perPage: number = 10,
  withPage: number = 1,
) => {
  const deleteWhereHouseParams = new IndexWhereHouseParams(query, pageNumber, perPage, withPage)
  await indexWhereHouseController.getData(deleteWhereHouseParams)
}
...
const searchWhereHouse = debounce(() => {
  fetchWhereHouse(word.value)
})

const deleteWhereHouse = async (id: number) => {
  const deleteWhereHouseParams = new DeleteWhereHouseParams(id)
  await DeleteWhereHouseController.getInstance().deleteWhereHouse(deleteWhereHouseParams)
  await fetchWhereHouse()
}

const handleChangePage = (page: number) => {
  currentPage.value = page
  fetchWhereHouse('', currentPage.value, countPerPage.value)
}
...
watch(
  () => indexWhereHouseController.state.value,
  (newState) => {
    if (newState) {
...
const actionList = (id: number, deleteWhereHouse: (id: number) => void) => [
  {
    ...
    permission: [
      ...
      PermissionsEnum.WHIERE_HOUSE_DETAILS,

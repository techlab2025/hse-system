...
export default class disActiveObjectivesController extends ControllerInterface<ObjectivesModel> {
  ...
  private disObjectivesUseCase = new DisObjectivesUseCase()

  static getInstance() {
    if (!this.instance) {
      this.instance = new disActiveObjectivesController()
    }
    return this.instance
  }

  async disActiveObjectives(params: Params) {
    // useLoaderStore().setLoadingWithDialog();
    // console.log(params)
    this.setLoading()
    const dataState: DataState<ObjectivesModel> = await this.disObjectivesUseCase.call(params)

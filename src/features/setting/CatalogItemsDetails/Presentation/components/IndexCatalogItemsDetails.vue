...
//
import DeleteCatalogItemsDetailsController from '../controllers/deleteCatalogItemsDetailsController'
import IndexCatalogItemsDetailsController from '../controllers/indexCatalogItemsDetailsController'
import IndexCatalogItemsDetailsParams from '../../Core/params/indexCatalogItemsDetailsParams'
import DeleteCatalogItemsDetailsParams from '../../Core/params/deleteCatalogItemsDetailsParams'
...
const indexCatalogItemsDetailsController = IndexCatalogItemsDetailsController.getInstance()
const state = ref(indexCatalogItemsDetailsController.state.value)
const route = useRoute()
...
const fetchCatalogItemsDetails = async (
  ...
  const indexCatalogItemsDetailsParams = new IndexCatalogItemsDetailsParams(query, pageNumber, perPage, withPage, id)
  await indexCatalogItemsDetailsController.getData(indexCatalogItemsDetailsParams)
}
...
const searchCatalogItemsType = debounce(() => {
  fetchCatalogItemsDetails(word.value)
})

const deleteCatalog = async (id: number) => {
  const deleteCatalogItemsParams = new DeleteCatalogItemsDetailsParams(id)
  await DeleteCatalogItemsDetailsController.getInstance().deleteCatalogItemsDetails(deleteCatalogItemsParams)
  await fetchCatalogItemsDetails()
}

const handleChangePage = (page: number) => {
  currentPage.value = page
  fetchCatalogItemsDetails('', currentPage.value, countPerPage.value)
}
...
watch(
  () => indexCatalogItemsDetailsController.state.value,
  (newState) => {
    if (newState) {
  ...
  <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 mb-4">
    ...
    <div class="col-span-2 flex justify-end gap-2">
      ...
      <permission-builder :code="[
        ...
        <router-link :to="`/${user?.type == OrganizationTypeEnum.ADMIN ? 'admin' : 'organization'}/catalog-items-details/add`"
          class="btn btn-primary">
          {{ $t('Add_Catalog_Item_Details') }}
        </router-link>
      </permission-builder>
    </div>
  </div>
          ...
          <table class="main-table">
            ...
            <tbody>
              <tr v-for="(item, index) in state.data" :key="item.id">
                <td data-label="#">
                  <router-link
                    :to="`/${user?.type == OrganizationTypeEnum.ADMIN ? 'admin' : 'organization'}/catalog-items-details/edit/${item.id}`">{{
                    index + 1 }}
                  </router-link>
                </td>

...
export default class EditCertificateController extends ControllerInterface<CertificateModel> {
  ...
  private EditCertificateUseCase = new EditCertificateUseCase()

  static getInstance() {
    if (!this.instance) {
      this.instance = new EditCertificateController()
    }
    return this.instance
  }

  async editCertificate(params: Params, router: any) {
    // useLoaderStore().setLoadingWithDialog();
    // console.log(params)
    try {
      const dataState: DataState<CertificateModel> = await this.EditCertificateUseCase.call(params)

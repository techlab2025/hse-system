...
import AddFactorExcelParams from '../../Core/params/addFactorExcelParams'
...
// ─── State ────────────────────────────────────────────────────────────────────
const sheetData = ref<HerikalyDetailsModel[] | null>(null)
...
const getBodyData = (data: any[]) => FactoryDetailsModel.transformData(data.slice(1))
...
const AddOrgEmployee = async () => {
  ...
  const orgData = new AddFactorExcelParams({ data: dataAsObjects })
  await addFactoryController.addFactory(orgData, router)
  if (addFactoryController.isDataSuccess()) {
    emit('uploaded')
  }
}

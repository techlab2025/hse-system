...
// ─── State ────────────────────────────────────────────────────────────────────
const sheetData = ref<HazardTypeModel[] | null>(null)
...
const getBodyData = (data: any[]) => HazardTypeModel.transformData(data.slice(1))
...
const AddHazardType = async () => {
  ...
  const orgData = new AddHazardTypeExcelParams({ data: dataAsObjects })
  await addHazardTypeController.addHazardType(orgData, router, route)
  ...
  <div class="page-wrapper">
    ...
    <template v-else>
      ...
      <template v-if="mappedData && mappedData.length > 0">
        ...
        <button @click="AddHazardType" class="btn-confirm">Confirm & Submit</button>
      </template>
    </template>
  </div>
</template>

...
export default class AddSystemHazardTypeController extends ControllerInterface<HazardTypeModel> {
  ...
  private addSystemHazardTypeUseCase = new AddSystemHazardTypeUseCase()

  static getInstance() {
    if (!this.instance) {
      this.instance = new AddSystemHazardTypeController()
    }
    return this.instance
  }

  async addSystemHazardType(params: any, router: Router, draft: boolean = false) {
    ...
    try {
      ...
      const dataState: DataState<HazardTypeModel> =
        await this.addSystemHazardTypeUseCase.call(params)

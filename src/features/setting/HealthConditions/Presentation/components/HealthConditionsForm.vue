...
// import { HealthConditionssMap } from '@/constant/HealthConditionss'
import LangTitleInput from '@/shared/HelpersComponents/LangTitleInput.vue'
import type ShowHealthConditionsModel from '@/features/setting/HealthConditions/Data/models/HealthConditionsDetailsModel'
import USA from '@/shared/icons/USA.vue'
import SA from '@/shared/icons/SA.vue'
import TranslationsParams from '@/base/core/params/translations_params.ts'
import EditHealthConditionsParams from '@/features/setting/HealthConditions/Core/params/editHealthConditionsParams.ts'
...
const props = defineProps<{
  data?: ShowHealthConditionsModel
}>()
...
// default available HealthConditionss
...
const fetchLang = async (
  ...
  const indexHealthConditionsController = await IndexLangController.getInstance().getData(params)

  const response = indexHealthConditionsController.value

  if (response?.data?.length) {
    // map backend HealthConditionss into default structure
    langDefault.value = response.data.map((item: any) => ({
      locale: item.code,
      title: '', // empty initially
      // if you already have icons mapped, use HealthConditionssMap
      icon: markRaw(LangsMap[item.code as keyof typeof LangsMap]?.icon),
    }))
  } else {
    langDefault.value = [
...
const updateData = () => {
  ...
  const params = props.data?.id
    ? new EditHealthConditionsParams(
      props.data?.id! ?? 0,
      translationsParams,
      AllIndustry,
      industry.value?.map((item) => item.id) ?? [],
    )
    : new AddHealthConditionsParams(
      translationsParams,
      AllIndustry,
      industry.value?.map((item) => item.id),
      // id,
...
// init HealthConditionss either from backend (edit mode) or from defaults (create mode)
watch(
...
<template>
  ...
  <div class="col-span-4 md:col-span-2" v-if="!allIndustries && user.user?.type == OrganizationTypeEnum.ADMIN">
    <CustomSelectInput :modelValue="industry" :controller="industryController" :params="industryParams" label="industry"
      id="HealthConditions" placeholder="Select industry" :type="2" @update:modelValue="setIndustry" />
  </div>

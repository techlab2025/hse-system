...
import IndexObserverationTypeController from '../controllers/indexObserverationTypeController'
...
const fetchObserverationType = async () => {
  const params = new IndexObserverationTypeParams('', 1, 10, 1)
  await indexObserverationTypeController.getData(params)
}
onMounted(fetchObserverationType)
...
const actionList = () => [
  {
    ...
    permission: [PermissionsEnum?.ORGANIZATION_EMPLOYEE, PermissionsEnum?.OBSERVATION_TYPE_CREATE],
  },
  {

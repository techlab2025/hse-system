...
import AddRootCausesController from '../controllers/addRootCausesController'
import AddRootCausesExcelParams from '../../Core/params/AddRootCausesExcelParams'
...
// ─── State ────────────────────────────────────────────────────────────────────
const sheetData = ref<RootCausesModel[] | null>(null)
...
const getBodyData = (data: any[]) => RootCausesModel.transformData(data.slice(1))
...
const AddRootCauses = async () => {
  ...
  const orgData = new AddRootCausesExcelParams({ data: dataAsObjects })
  await addRootCausesController.addRootCauses(orgData, router)
}
  ...
  <div class="page-wrapper">
    <div class="excel-warning">
      ...
      <div class="rule-group">
        ...
        <div class="field-tags">
          <span class="field-tag">Root Cause Name</span>
        </div>
      </div>
      <hr class="separator" />
    </div>
    ...
    <template v-else>
      ...
      <template v-if="mappedData && mappedData.length > 0">
        ...
        <button @click="AddRootCauses" class="btn-confirm">Confirm & Submit</button>
      </template>
    </template>
  </div>
</template>

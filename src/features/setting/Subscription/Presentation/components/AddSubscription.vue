...
import type AddSubscriptionParams from '../../Core/params/addSubscriptionParams'
...
const saveAndNew = async () => {
  console.log(params.value, 'params')
  addSubscriptionController.setLoading()
  await addSubscriptionController.addSubscription(
    params.value as AddSubscriptionParams,
    stayOnPageRouter,
  )
  if (addSubscriptionController.isDataSuccess()) {
    params.value = null
    formKey.value++
  }
}
...
<template>
  <form class="grid grid-cols-1 md:grid-cols-4 gap-8" @submit.prevent="addSubscription">
    <SubscriptionForm :key="formKey" @update:data="setParams" />

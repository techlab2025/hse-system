...
import EditHeaderParams from '../../Core/params/editHeaderParams'
import AddHeaderParams from '../../Core/params/addHeaderParams'
// import { filesToBase64 } from '@/base/Presentation/utils/file_to_base_64.ts'

const emit = defineEmits(['update:data'])

const props = defineProps<{
  data?: HeaderDetailsModel
}>()
...
const fetchLang = async (
  ...
  const indexHeaderController = await IndexLangController.getInstance().getData(params)

  const response = indexHeaderController.value

  if (response?.data?.length) {
    // map backend Headers into default structure
    langDefault.value = response.data.map((item: any) => ({
      locale: item.code,
      title: '', // empty initially
      // if you already have icons mapped, use HeadersMap
...
// init Headers either from backend (edit mode) or from defaults (create mode)
watch(

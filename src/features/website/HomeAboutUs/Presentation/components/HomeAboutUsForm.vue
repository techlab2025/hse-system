...
import AddHomeAboutUsParams from '../../Core/params/addHomeAboutUsParams'
import EditHomeAboutUsParams from '../../Core/params/editHomeAboutUsParams'
import SingleFileUpload from '@/shared/HelpersComponents/SingleFileUpload.vue'

const emit = defineEmits(['update:data'])

const props = defineProps<{
  data?: HomeAboutUsDetailsModel
}>()
...
const updateData = () => {
  ...
  const itemsParams = items.value.map((item) => {
    ...
    const params = new AddHomeAboutUsParams(itemTranslations, item.alt_image, item.image)
    return params
  })

  const params = props.data?.id
    ? new EditHomeAboutUsParams(
        props.data?.id ?? 0,
        translationsParams,
        alt_image.value,
        image.value,
    ...
    : new AddHomeAboutUsParams(translationsParams, alt_image.value, image.value, itemsParams)
...
watch(
  ...
  ([newData, newDefault]) => {
    ...
    items.value = newData?.HomeAboutUsItems?.length
      ? newData.HomeAboutUsItems.map((it: any) => ({
...
<template>
  ...
  <div class="col-span-4 md:col-span-2 input-wrapper">
    ...
    <input type="text" v-model="alt_image" />
  </div>

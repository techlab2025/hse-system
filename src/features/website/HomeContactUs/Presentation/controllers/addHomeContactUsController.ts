...
export default class AddHomeContactUsController extends ControllerInterface<HomeContactUsModel> {
  ...
  private AddHomeContactUsUseCase = new AddHomeContactUsUseCase()

  static getInstance() {
    if (!this.instance) {
      this.instance = new AddHomeContactUsController()
    }
  ...
  async addHomeContactUs(params: Params, router: Router, draft: boolean = false) {
    ...
    try {
      ...
      if (this.isDataSuccess()) {
        ...
        if (!draft) await router.push('/admin/home-contact-us')

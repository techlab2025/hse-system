    ...
    <DataStatus :controller="state">
          ...
          <table class="main-table">
            ...
            <tbody>
              <tr v-for="(item, index) in state.data" :key="item.id">
                <td data-label="#">
                  <router-link :to="`/admin/power-full/${item.id}`">{{ index + 1 }} </router-link>
                </td>
      ...
      <template #empty>
        <DataEmpty :link="`/admin/power-full/add`" addText="Add PowerFull"
          description="You have no PowerFull .. All your joined customers will appear here when you add your customer data"
          title="You have No PowerFull" />
      </template>
      <template #failed>

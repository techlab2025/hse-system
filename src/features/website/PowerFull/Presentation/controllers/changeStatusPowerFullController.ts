...
export default class ChangeStatusPowerFullController extends ControllerInterface<PowerFullModel> {
  ...
  private ChangeStatusPowerFullUseCase = new ChangeStatusPowerFullUseCase()

  static getInstance() {
    if (!this.instance) {
      this.instance = new ChangeStatusPowerFullController()
    }
    return this.instance
  }

  async changeStatusPowerFull(params: Params) {
    // useLoaderStore().setLoadingWithDialog();
    // console.log(params)
    this.setLoading()
    const dataState: DataState<PowerFullModel> =
      await this.ChangeStatusPowerFullUseCase.call(params)

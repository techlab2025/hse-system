...
import IndexSystemBannerController from '../controllers/indexSystemBannerController'
import IndexSystemBannerParams from '../../Core/params/indexSystemBannerParams'
import DeleteSystemBannerParams from '../../Core/params/deleteSystemBannerParams'
import DeleteSystemBannerController from '../controllers/deleteSystemBannerController'
import ChangeSystemBannerStatusParams from '../../Core/params/changeStatusSystemBannerParams'
import disActiveSystemBannerController from '../controllers/disActiveSystemBannerController'
...
const indexSystemBannerController = IndexSystemBannerController.getInstance()
const state = ref(indexSystemBannerController.state.value)
const route = useRoute()
const id = route.params.parent_id
// const type = ref<ServiceStatusEnum>(ServiceStatusEnum[route.params.type as keyof typeof ServiceStatusEnum])

const fetchSystemBanner = async (
  query: string = '',
  pageNumber: number = 1,
  perPage: number = 10,
  withPage: number = 1,
) => {
  const indexSystemBannerParams = new IndexSystemBannerParams(
    query,
    pageNumber,
    perPage,
    withPage,
  ...
  await indexSystemBannerController.getData(indexSystemBannerParams)
}
...
const handleChangePage = (page: number) => {
  ...
  fetchSystemBanner('', currentPage.value, countPerPage.value)
}
...
watch(
  () => indexSystemBannerController.state.value,
  (newState) => {
    if (newState) {
...
const changeStatusSystemBanner = async (id: number) => {
  const changeSystemBannerStatusParams = new ChangeSystemBannerStatusParams(id)
  await disActiveSystemBannerController
    .getInstance()
    .disActiveSystemBanner(changeSystemBannerStatusParams)
  await fetchSystemBanner()
}
</script>
...
<template>
  <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 mb-4">
    ...
    <div class="col-span-2 flex justify-end gap-2">
      ...
      <ExportPdf />
      <PermissionBuilder :code="[PermissionsEnum.WEBSITE, PermissionsEnum.OUR_SYSTEM_BANNER_CREATE]">

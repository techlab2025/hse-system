...
import DeleteSystemComponentController from '../controllers/deleteSystemComponentController'
import ChangeSystemComponentStatusParams from '../../Core/params/changeStatusSystemComponentParams'
import disActiveSystemComponentController from '../controllers/disActiveSystemComponentController'
...
const indexSystemComponentController = IndexSystemComponentController.getInstance()
const state = ref(indexSystemComponentController.state.value)
const route = useRoute()
const id = route.params.parent_id
// const type = ref<ServiceStatusEnum>(ServiceStatusEnum[route.params.type as keyof typeof ServiceStatusEnum])

const fetchSystemComponent = async (
  query: string = '',
  pageNumber: number = 1,
  perPage: number = 10,
  withPage: number = 1,
) => {
  const indexSystemComponentParams = new IndexSystemComponentParams(
    query,
    pageNumber,
    perPage,
    withPage,
  ...
  await indexSystemComponentController.getData(indexSystemComponentParams)
}

onMounted(() => {
  fetchSystemComponent()
})

const searchService = debounce(() => {
  fetchSystemComponent(word.value)
})

const deleteService = async (id: number) => {
  const deleteSystemComponentParams = new DeleteSystemComponentParams(id)
  await DeleteSystemComponentController.getInstance().deleteSystemComponent(
    deleteSystemComponentParams,
  )
  await fetchSystemComponent()
}

const handleChangePage = (page: number) => {
  currentPage.value = page
  fetchSystemComponent('', currentPage.value, countPerPage.value)
}
...
const changeStatusSystemComponent = async (id: number) => {
  const changeSystemComponentStatusParams = new ChangeSystemComponentStatusParams(id)
  await disActiveSystemComponentController
    .getInstance()
    .disActiveSystemComponent(changeSystemComponentStatusParams)
  await fetchSystemComponent()
}
</script>

...
import EditAboutUsFeatureParams from '../../Core/params/editSystemRiskManagementParams'
import AddAboutUsFeatureParams from '../../Core/params/addSystemRiskManagementParams'
...
const props = defineProps<{
  data?: AboutUsFeatureDetailsModel
}>()
...
// default available AboutUsFeatures
...
const fetchLang = async (
  ...
  const indexAboutUsFeatureController = await IndexLangController.getInstance().getData(params)

  const response = indexAboutUsFeatureController.value

  if (response?.data?.length) {
    // map backend AboutUsFeatures into default structure
    langDefault.value = response.data.map((item: any) => ({
      locale: item.code,
      title: '', // empty initially
      // if you already have icons mapped, use AboutUsFeaturesMap
...
// init AboutUsFeatures either from backend (edit mode) or from defaults (create mode)
watch(

...
import IndexSystemRiskTypeParams from '../../Core/params/indexSystemRiskTypeParams'
import DeleteSystemRiskTypeParams from '../../Core/params/deleteSystemRiskTypeParams'
import DeleteSystemRiskTypeController from '../controllers/deleteSystemRiskTypeController'
import ChangeSystemRiskTypeStatusParams from '../../Core/params/changeStatusSystemRiskTypeParams'
import disActiveSystemRiskTypeController from '../controllers/disActiveSystemRiskTypeController'
...
const indexSystemRiskTypeController = IndexSystemRiskTypeController.getInstance()
const state = ref(indexSystemRiskTypeController.state.value)
const route = useRoute()
const id = route.params.parent_id
// const type = ref<ServiceStatusEnum>(ServiceStatusEnum[route.params.type as keyof typeof ServiceStatusEnum])

const fetchSystemRiskType = async (
  query: string = '',
  pageNumber: number = 1,
  perPage: number = 10,
  withPage: number = 1,
) => {
  const indexSystemRiskTypeParams = new IndexSystemRiskTypeParams(
    query,
    pageNumber,
    perPage,
    withPage,
  ...
  await indexSystemRiskTypeController.getData(indexSystemRiskTypeParams)
}
...
const searchService = debounce(() => {
  fetchSystemRiskType(word.value)
})

const deleteService = async (id: number) => {
  const deleteSystemRiskTypeParams = new DeleteSystemRiskTypeParams(id)
  await DeleteSystemRiskTypeController.getInstance().deleteSystemRiskType(
    deleteSystemRiskTypeParams,
  )
  await fetchSystemRiskType()
}

const handleChangePage = (page: number) => {
  currentPage.value = page
  fetchSystemRiskType('', currentPage.value, countPerPage.value)
}
...
watch(
  () => indexSystemRiskTypeController.state.value,
  (newState) => {
    if (newState) {
...
const actionList = (id: number, deleteService: (id: number) => void) => [
  {
    text: t('edit'),
    icon: IconEdit,
    link: `/admin/system_risk_type/${id}`,
    permission: [
...
const changeStatusSystemRiskType = async (id: number) => {
  const changeSystemRiskTypeStatusParams = new ChangeSystemRiskTypeStatusParams(id)
  await disActiveSystemRiskTypeController
    .getInstance()
    .disActiveSystemRiskType(changeSystemRiskTypeStatusParams)
  await fetchSystemRiskType()
}
</script>

...
import type PowerFullDetailsModel from '../../Data/models/SystemWorkDetailsModel'
import EditPowerFullParams from '../../Core/params/editSystemWorkParams'
import AddPowerFullParams from '../../Core/params/addSystemWorkParams'
...
const fetchLang = async (
  ...
  const indexPowerFullController = await IndexLangController.getInstance().getData(params)

  const response = indexPowerFullController.value

  if (response?.data?.length) {
    // map backend PowerFulls into default structure
    langDefault.value = response.data.map((item: any) => ({
      locale: item.code,
      title: '', // empty initially
      // if you already have icons mapped, use PowerFullsMap
...
// init PowerFulls either from backend (edit mode) or from defaults (create mode)
watch(

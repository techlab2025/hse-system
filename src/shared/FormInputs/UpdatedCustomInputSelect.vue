...
interface Props {
   ...
   * when component type is multiselect.
   *
   * null = unlimited
   */
...
/*
 * These props will only be passed when the component
 * is working as a MultiSelect.
 */
const multiselectProps = computed(() => {
  if (!isMultiselect.value) {
    return {}
  ...
  return {
    ...
    /*
     * PrimeVue MultiSelect selection limit.
     *
     * Example:
     * maxSelectOptions = 3
     * => user can select maximum 3 options.
     *
     * null
     * => unlimited selection.
     */
    selectionLimit: props.maxSelectOptions ?? null,
  }
})
...
async function fetchOptions(): Promise<void> {
  ...
  try {
    ...
    /*
     * Ignore the result when a newer request
     * has already started.
     */
    if (requestId !== currentRequestId) {
      return
...
async function reloadData(): Promise<void> {
  ...
  /*
   * Clear the selected value before fetching.
   * This allows autoFill to select the only
   * available option afterward.
   */
  normalizedValue.value = isMultiselect.value ? [] : null
        ...
        <template #option="{ option }">
          <slot name="option" :option="option">
            <span v-if="showOptionSerial || showOptionLicensePlate" class="equipment-option">
              ...
              <small v-if="showOptionSerial && getOptionSerial(option)">
                {{ $t('serial_number') }}:
                {{ getOptionSerial(option) }}
              </small>

              <small v-else-if="showOptionLicensePlate && getOptionLicensePlate(option)">
                {{ $t('License Plate Number') }}:
                {{ getOptionLicensePlate(option) }}
              </small>
            </span>
